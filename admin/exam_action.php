			...
			if($total_subject > 0)
			{
                ...
                else
                {
                	if(time() > $exam_last_subject_end_datetime)
                	{
	                    ...
	                    $action_button = '<button type="button"  name="publish_result" class="btn btn-primary btn-sm publish_result" target="_blank" data-exam_id="'.$row["exam_id"].'">Publish Result</button>&nbsp;&nbsp;<a href="exam_result.php?ec='.$row["exam_code"].'" class="btn btn-secondary btn-sm">View Result</a>';
	                }
	                else
	                {
